    ...
    public class GetInvoiceStatusCountController : ApiController
    {
        ...
        public IHttpActionResult GetInvoiceCountStatus(string firstdate, string lastdate, string segment)
        {
            try
            {
                ...
                string queryPDFsCreated = "SELECT COUNT(InvoicePdfStatus) FROM invoice_generation_table WHERE DocumentDate >= @firstdate AND DocumentDate <= @lastdate AND segmentCode = @segment AND InvoicePdfStatus = 'X'";
                ...
                string queryDigitallySigned = "SELECT COUNT(InvoicePdfDigitalSigStatus) FROM invoice_generation_table WHERE DocumentDate >= @firstdate AND DocumentDate <= @lastdate AND segmentCode = @segment and InvoicePdfDigitalSigStatus = 'X'";
